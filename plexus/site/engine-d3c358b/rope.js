...
function checkPair(ya, yb, minCos) {
  const signOk = ya * yb >= 0;
  const den = Math.hypot(1, ya) * Math.hypot(1, yb);
  const cosOk = den > 0 ? Math.abs((1 + ya * yb) / den) >= minCos : true;
  return { signOk, cosOk };
}

/** All strand pairs in g co-directed at bar i (same rule as together angle).
 * Gate returns boolean, aborting at the first failing pair as before.
...
export function groupAligned(ma, g, i, atr, cfg) {
  ...
  for (let a = 0; a < slopes.length; a++)
    for (let b = a + 1; b < slopes.length; b++) {
      const { signOk, cosOk } = checkPair(slopes[a], slopes[b], minCos);
      if (signOk === false || cosOk === false) return false;
    }
  return true;
}

/** Full two-pass alignment diagnostic for measurement (S10-10 1a). All pairs
 * are checked in each pass (sign first, then angle), so the split is complete
 * and independent of pair order. Categories are mutually exclusive and must sum
 * to the number of candidates fed in:
 *   degenerate = early exit (first bar / single strand / no ATR / missing value)
 *   sign       = at least one pair has opposite-sign slopes
 *   angle      = sign agreed everywhere but a pair fails the angle bound
 *   passed     = aligned on both sign and angle
...
export function groupAlignedDiagnostic(ma, g, i, atr, cfg) {
  ...
  for (let a = 0; a < slopes.length; a++)
    for (let b = a + 1; b < slopes.length; b++)
      if (checkPair(slopes[a], slopes[b], minCos).signOk === false)
        return { category: "sign", reason: "oppositeSign" };
  for (let a = 0; a < slopes.length; a++)
    for (let b = a + 1; b < slopes.length; b++)
      if (checkPair(slopes[a], slopes[b], minCos).cosOk === false)
        return { category: "angle", reason: "angleExceeded" };
